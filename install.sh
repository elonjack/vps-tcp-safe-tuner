...
readonly EXPECTED_SHA256='e371b19aa52154db24e587dc45613ce748ab37d2ab5288a35005d5e66e2d9b3a'
...
if [[ -t 0 && -t 1 ]]; then
  say '正在启动中文调优菜单。'
  if [[ ${EUID:-$(id -u)} -eq 0 ]]; then "$INSTALL_PATH"; else sudo "$INSTALL_PATH"; fi
  exit 0
fi
say '非交互安装完成；运行：sudo vps-tcp-tune'
